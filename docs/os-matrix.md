...
| Linux | Planning phases only (`/greenloop:idea` → `/greenloop:architecture` need nothing but Claude Code) | ✅ Full once the `maestro-android` profile lands — **the first post-v1 milestone** |
...
Portability groundwork already in place: every driver (`greenloop-verify`, `greenloop-doctor`,
`greenloop-loop`, the Stop hook, the installer) is Node, not bash; the E2E device target is a
