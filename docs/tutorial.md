# Tutorial: idea → verified app with Breathe-and-Build
...
instructions and re-checks until green — it never half-installs. Then it registers the `bnb`
plugin and the Expo MCP server (`/mcp` in your next session to authenticate). Use
`npx breathe-and-build --check` any time for a report-only preflight.

## 1. Breathe: `/bnb:idea`

Open Claude Code in an empty directory and run `/bnb` — it tells you you're in the idea
phase. `/bnb:idea` is a free-form conversation: describe the receptionist, the queue at the
desk, the one job (add a patient, see the list). It ends with a `prd.md` v0 draft you
confirm.

## 2. Breathe: `/bnb:prd` — the gap-hunting loop
...
Runs `bnb-doctor` first (fix-it list until clean), then births the monorepo from the profile
...
## 5. Build: `/bnb:feature s01-patient-list`
...
fix, retry. Exit 3 → environment, not code: run `bnb-doctor`. The session cannot end while
the task is red — that's the Stop hook.

## 6. Overnight variant

`/bnb:overnight s02-...` queues well-specified tasks and launches
`timeout 30m bnb-loop <task> 20` laps. In the morning, the wake-up ritual: review the full
diff, re-run `bnb-verify` yourself. Only a fresh exit 0 counts.
