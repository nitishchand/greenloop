...
Phases: implement the feature -> author/repair its E2E flow -> run greenloop-verify ${taskId} ->
...
best answer consistent with it. Show the greenloop-verify exit code as evidence.
Only print DONE when greenloop-verify ${taskId} exits 0.`;
}

export function runLoop(taskId, { maxLaps = 20, lap, verify, log = console.error }) {
  for (let n = 1; n <= maxLaps; n += 1) {
    log(`==== greenloop-loop lap ${n}/${maxLaps} (${taskId}) ====`);
    try {
      lap(taskId, n);
    } catch (err) {
